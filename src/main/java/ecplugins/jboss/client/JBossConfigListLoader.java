...
// JBossConfigListLoader.java --
//
// JBossConfigListLoader.java is part of ElectricCommander.
//
// Copyright (c) 2005-2012 Electric Cloud, Inc.
// All rights reserved.
//

package ecplugins.jboss.client;
...
public class JBossConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final JBossConfigList m_configList;
    private final CgiRequestProxy m_cgiRequestProxy;
    private String                m_editorName;

    //~ Constructors -----------------------------------------------------------

    public JBossConfigListLoader(
            JBossConfigList configList,
            Component       component,
            ChainedCallback callback)
    {
        super(component, callback);
        m_configList      = configList;
        m_cgiRequestProxy = new CgiRequestProxy(getPluginName(), "jboss.cgi");
    }
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ...
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading JBoss configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ...
                // noinspection HardCodedStringLiteral
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading JBoss configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-JBoss/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("jbosscfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for JBoss plugin '" + m_configPlugin + "'";
