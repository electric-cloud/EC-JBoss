...
sub getStepParameters {
    ...
    if ($context->isCheckConnectionInCreateOrEditConfigurationContext()) {
...
sub getCurrentStepParametersAsHashref {
    ...
    for my $param ($xpath->findnodes('//formalParameter')) {
        ...
        if ($param->findvalue('type')->string_value() eq 'credential') {
            if (!$value) {
                fwLogDebug("Empty value for credential '$name' has found, skipping it.");
                next;
            }
...
# private
sub isCheckConnectionInCreateOrEditConfigurationContext {
    my ($self) = @_;

    my $procedureName = $self->getProcedureName();
    my $stepName = $self->getStepName();
    if ($stepName eq 'checkConnection') {
        if ($procedureName eq 'CreateConfiguration' || $procedureName eq 'EditConfiguration') {
            return 1;
        }
        return 0;
    }
