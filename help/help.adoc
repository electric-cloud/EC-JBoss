...
A unique name of the configuration to be used. URL, port and credentials are retrieved from the given configuration. To view or create a new configuration, go to the Administration -> Plugins tab, and select 'Configure' action for EC-JBoss plugin. NOTE: information for connecting to Master CLI should be provided within the specified configuration, even for stating slave host controllers.
...
|Domain Configuration File |Optional Parameter. Applies only to master host controllers.
...
'Additional options' to be passed on to domain startup script. If provided parameters will be concatenated AS IS to already generated command line for starting master or slave host controller. These are some examples.
